...
namespace ManagerDirectory.Services
{
    public class RemovingService
    {
        ...
        private async Task DeleteAsync()
        {
            ...
            if (_countFiles != 0)
            {
	            var current = Directory.EnumerateFiles(_fullPathDirectory, "*.*", SearchOption.AllDirectories)
            ...
            {
	            var current = Directory.EnumerateDirectories(_fullPathDirectory, "*", SearchOption.AllDirectories)
		            .GetEnumerator().Current;
	            if (current != null)
